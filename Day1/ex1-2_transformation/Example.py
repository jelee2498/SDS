...
while iTransformed < iMax:
    ...
    while jTransformed < jMax:
        ...
        nativeI, nativeJ, _ = np.dot(T_inv, np.transpose([iTransformed, jTransformed, 1]))
        ...
        imageTransformed[iTransformed, jTransformed] = ndimage.map_coordinates(image, [[nativeI], [nativeJ]], order=1)
        jTransformed += 1
    iTransformed += 1
